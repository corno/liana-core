...
import type * as s_function from "../../../schemas/unmarshalling.js"
...
import * as t_from_loc from "../../deserializers/primitives.js"
import type { s } from 'pareto-core/implementation/command'

export const Number: declarations.Number = ($, abort, $p) => {
    const value = $

    const as_text = api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree'].Text(
        $,
            ...
            switch ($[0]) {
                case 'binary': return p_.option($, ($) => t_from_loc.binary(
                    ...
                    ($) => abort(['liana', {
                        ...
                        'range': api_astn_core.api.deserialization.transformers['parse tree']['start token range'].Value(value)
                    }]),
                ))
...
export const Dictionary: declarations.Dictionary = ($, abort) => {
    return p_change_context(
        api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree'].Dictionary($, ($) => abort(['astn value unmarshalling', $])),
        ($) => {
            const value = $.value
            return {
                'value': $.value,
...
export const List: declarations.List = ($, abort) => {
    return api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree'].List($, ($) => abort(['astn value unmarshalling', $]))
}

export const Nothing: declarations.Nothing = ($, abort) => api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree'].Nothing($, ($) => abort(['astn value unmarshalling', $])).null

export const Optional: declarations.Optional = ($, abort) => api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree'].Optional($, ($) => abort(['astn value unmarshalling', $]))
...
export const State: declarations.State = ($, abort) => api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree'].State($, ($) => abort(['astn value unmarshalling', $]))

export const Text: declarations.Text = ($, abort) => api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree'].Text($, ($) => abort(['astn value unmarshalling', $])).token.value

export const Verbose_Group: declarations.Verbose_Group = ($, abort, $p) => {
    return p_change_context(
        api_astn_core.api.unmarshalling.refiners['unmarshalled value']['parse tree']['Verbose Group']($, ($) => abort(['astn value unmarshalling', $]), $p),
        ($) => {
            const value = $.value
            return {
                'value': $.value,
