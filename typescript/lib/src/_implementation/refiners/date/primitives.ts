...
import type * as s_date from "../../../private_schemas/date.js"

export const iso_date_udhr: p_.Refiner_Without_Error<
    s_date.Date,
    number
> = ($) => {
    ...
    const $v_number_of_400_year_blocks = p_t.from.number(total_days).divide(
    ...
    const number_of_100_year_blocks = p_t.from.number($v_remaining_days_in_the_last_400_years).divide(
    ...
    const number_of_4_year_blocks = p_t.from.number($v_remaining_days_in_the_last_100_years).divide(
    ...
    const number_of_1_year_blocks = p_t.from.number($v_remaining_days_in_the_last_4_years).divide(
