...
import * as t_unmarshalling_to_location from "../unmarshalling/location.js"

export const Error: declarations.Error = ($) => p_.from.state($).decide(
    ($) => {
        switch ($[0]) {
            case 'parse tree deserialization': return p_.option($, ($) => api_astn_core.api.transformers['parse tree deserialization'].location.Error($))
